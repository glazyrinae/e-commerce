document.addEventListener("DOMContentLoaded", () => {
  ...
  const syncRowButtons = (row) => {
    ...
    if (minusBtn) minusBtn.classList.toggle("disabled", qty <= 1);
    if (plusBtn) plusBtn.classList.toggle("disabled", stock <= 0 || qty >= stock);
  };

  const lockRow = (row, isLocked) => {
    row.dataset.loading = isLocked ? "1" : "0";
    const controls = row.querySelectorAll(".js-qty-btn, .js-remove-item");
    controls.forEach((control) => control.classList.toggle("disabled", isLocked));
  };
